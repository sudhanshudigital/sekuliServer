...
public class SikuliServer {
	...
	public static void unTar(String tarFileLocation, String unTarFilesDir)
			throws IOException {
		...
		while ((entry = (TarArchiveEntry) tarIn.getNextEntry()) != null) {
			...
			if (entry.isDirectory()) {
				...
				FileOutputStream fos = new FileOutputStream(unTarFilesDir
						+ entry.getName());
	...
	public static int click(String image1) {
		int flag = 0;
		try {
			System.out.println("IMagedir== " + imagegDir + image1);
	...
	/*
	 * 
	 * Main method which is responsible for handling AllREST calls and uploading
	...
	public static void main(String[] args) {
		...
		post("/seleniumServer/*", (request, response) -> {
			...
			String seleniumUrl = "http://" + remoteIP + ":4444/wd/hub/"
					+ seleniumUri;
		...
		post("/sikuliServer", (request, response) -> {
			...
			case "initilaiseImages": {
				...
				String tarFileLocation = currentDir
						+ "/imagesDir/images.tar.gz";
